    ...
    void Update()
    {
        ...
        if(taskReady && mousePress)
        {
            ...
            if(mouseY>=0 && mouseYprev<=0)
            {
                ...
                float YDistToStart = Mathf.Abs(mouseY-startY);
                float PositiveNegative = Mathf.Sign(mouseXprev-mouseX);
                StartPoint = new Vector2(mouseX+PositiveNegative*XDist*(YDistToStart/YDist),startY);
                ...
                {
                    //スタートターゲットを通過したら
                    if(StartPoint.x<=startXRight && StartPoint.x>=startXLeft)
                    {
                        Ae = 0f;
        ...
        { 
            ...
            if(mouseY<0)
            {
                //ゴール地点を2点（）現在位置と1フレーム前位置から計測
                float YDist = Mathf.Abs(mouseYprev-mouseY);
                float XDist = Mathf.Abs(mouseX-mouseXprev);
                float YDistToStart = Mathf.Abs(mouseY-goalY);
                float PositiveNegative = Mathf.Sign(mouseXprev-mouseX);
                EndPoint = new Vector2(mouseX+PositiveNegative*XDist*(YDistToStart/YDist),goalY);
                /*Debug.Log("mouseX"+mouseX);
                Debug.Log("mouseY"+mouseY);
                Debug.Log("mouseXprev"+mouseXprev);
                Debug.Log("mouseYprev"+mouseYprev);
                Debug.Log("goalX"+EndPoint.x);
                Debug.Log("goalY"+EndPoint.y);*/
